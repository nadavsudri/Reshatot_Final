...
class ReliableUDP:
    def __init__(self, sock:socket.socket, window_size=20, max_msg_size=1024, timeout=2, dynamic_message_size=False):
        self.sock = sock
        self.peer_addr = None 
        self.config = {
    ...
    def connect(self, ip, port):
        ...
        self.sock.sendto(json.dumps(self.config).encode(), self.peer_addr)
    ...
    def close(self):
        self.sock.sendto("FIN".encode(), self.peer_addr)
        self.sock.settimeout(self.timeout)
        try:
    ...
    def send(self, source:str):
        ...
        self.sock.settimeout(0.5)  # Shorter timeout for recv attempts
        new_size = False
        pending_size = None
        window=[]
        timer = time.time()
        ack_timeout = time.time()  # FIX: Track when we last received an ACK
        ...
        while True:
            ...
            while last_sent-last_ack < window_size:
                ...
                self.sock.sendto(to_send + b"\n", self.peer_addr)
                ### proccess the ack
                # time.sleep(0.001) #uncomment for low quality
            ...
            while last_ack < last_sent:
                ...
                if res is not None:
                    ...
                    ack = res["ack"]
                    print("got Ack: ", ack)
                    ack_timeout = time.time()  # Reset timeout on ACK received

                    if res.get("dynamic_message_size"):
                        dynamic = res["dynamic_message_size"]
                        if max_len != res.get("message_size"):
                            new_size = True
                    ...
                    # FIX: Only timeout if we haven't received ANY ACK for a while
                    if time.time() - ack_timeout > self.timeout:
                        print("ACK Timeout - no ACKs received, retransmitting...")
                        break
                    continue
            ...
            if time.monotonic() - timer > self.timeout:
                ...
                for unacked in window:
                    self.sock.sendto(unacked + b"\n", self.peer_addr)
                timer = time.monotonic()
                ack_timeout = time.time()  # Reset ACK timeout on retransmit
